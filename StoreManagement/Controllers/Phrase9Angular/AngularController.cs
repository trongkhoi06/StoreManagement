...
namespace StoreManagement.Controllers
{
  ...
  public class AngularController : ApiController
  {
        ...
        if (start.Year > 1900)
        {
          ...
          if (sessionNum == 1)
          {
            ...
            foreach (var ss in adjustingSessions)
            {
              ...
              double adjustedQuantity = entry.Quantity;
              if (entry.KindRoleName == "AdjustingMinus")
              {
                adjustedQuantity *= -1;
              }
    ...
    public IHttpActionResult GetBoxesInformation()
    {
      ...
      try
      {
        List<Box> boxes = db.Boxes.Where(unit => unit.IsActive == true && unit.BoxID != "ĐÃ TRẢbox")
        ...
        foreach (var box in boxes)
        {
          ...
          else if (boxDAO.IsStoredCase(box.BoxPK))
          {
            ...
            foreach (var item in listItemPK)
            {
              if (new StoringDAO().AvailableQuantity(sBox, item.Item1, item.Item2) > 0)
              {
                count++;
              }
            }
    ...
    public IHttpActionResult GetSessionBySessionPKVerification(int sessionPK, int sessionNum)
    {
      try
      {
        ...
        if (sessionNum == 1)
        {
          ...
          SystemUser systemUser = db.SystemUsers.Find(ss.UserID);
          double temp = entry.Quantity;
          if (entry.KindRoleName == "AdjustingMinus")
          {
            temp *= -1;
          }

          result = new Client_Session_Detail_Verification_Angular(temp
        ...
        else
        {
          ...
          SystemUser systemUser = db.SystemUsers.Find(ss.UserID);
          result = new Client_Session_Detail_Verification_Angular(entry.Quantity
              , systemUser.Name + " (" + ss.UserID + ")", ss.ExecutedDate, ss.IsVerified);
