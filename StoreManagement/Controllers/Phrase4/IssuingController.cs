  ...
  public class IssuingController : ApiController
  {
    ...
    public IHttpActionResult GetIssueByUserWorkplace(string userID)
    {
      ...
      try
      {
        ...
        List<Demand> demands = db.Demands.Where(unit => workplace.WorkplacePK == unit.WorkplacePK).ToList();

        foreach (var demand in demands)
        {
          List<Issue> issues = db.Issues.Where(unit => unit.DemandPK == demand.DemandPK).ToList();
          foreach (var issue in issues)
          {
            result.Add(new Client_Issue1(issue, demand, workplace));
          }
